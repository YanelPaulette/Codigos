...
int insertar(int a[], int *len, int valor, int pos_a_insertar)  // inserta un valor en una posicion 
{
...
int insertarEnOrden(int a[], int* len, int valor)
{
  ...
  // {1,10,14,31} , len=4 , valor=5
    // i=0 , 0<4 && 1<=5  => i+=1
    // i=1 , 1<4 && 10<=5  
  while( i<*len && a[i]<=valor ) {   // con uno comparo la long, el otro el valor. 
    i = i + 1;
  }
